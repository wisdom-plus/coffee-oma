Review.create!(
  ...
  title: '最高のドリッパーです',
  content: '簡単においしいコーヒーを淹れることができいいドリッパーです',
  rate: 5
)
...
Review.create!(
  ...
  title: 'ドリップが安定する',
  content: "ウェーブドリッパーは、他のレビューにあるように、確実にドリップのブレが無いように感じます。
  常に安定したドリップになります。
...
Review.create!(
  ...
  title: '見た目',
  content: '見た目は　おしゃれだが　高く勝手良くない　落ちるのが早いので　コクも香りも深くない　一人分は三つ穴が　いいかな　浅煎りが好きなので　好き好きです　その日の気分で　使い分けてます',
  rate: 2
)
Review.create!(
  user: User.find(8),
  product: Product.find(15),
  title: 'よく考えられたもの',
  content: '普段使いのドリッパーが割れた��め、探して見たところとても気になったので購入。単純な構造だけど美しい、でも味はどうかな…なんて考えてましたがまったく問題はありませんでした。骨組みだけなので洗った感はでませんがよく考えられたものだと感心してます',
  rate: 5
)
Review.create!(
  user: User.find(9),
