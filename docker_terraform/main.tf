...
resource "aws_elasticache_subnet_group" "portfolio-subnet-group" {
  ...
  subnet_ids = [data.aws_subnet.portfolio-private-subnet-1.id, data.aws_subnet.portfolio-private-subnet-2.id]
}
...
module "redis_sg" {
  ...
  name        = "redis-sg"
  vpc_id      = data.aws_vpc.portfolio-vpc.id
  port        = 6379
  cidr_blocks = [data.aws_vpc.portfolio-vpc.cidr_block]
}
...
resource "aws_vpc_endpoint_route_table_association" "private_s3" {
  ...
  route_table_id  = data.aws_route_table.private-route-table-1.id
}
...
resource "aws_vpc_endpoint" "dkr" {
  ...
  subnet_ids          = [data.aws_subnet.portfolio-private-subnet-1.id]
